...
export default function ColorMatch(_props: { slug: string }) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 rounded-lg border border-neutral-800 bg-neutral-900 p-8 text-center">
        ...
        <p className="max-w-sm text-sm text-neutral-400">
          A color word appears, printed in its own ink color &mdash; tap
          Match if the ink matches what the word says, No Match if it
          doesn&apos;t. Answer before the bar runs out; correct streaks make
          it shorter. You have 3 lives; a wrong or missed answer costs one.
        </p>
        <button
          type="button"
          onClick={handleStart}
